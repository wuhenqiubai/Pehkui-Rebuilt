...
On NeoForge, rules can use the `neoforge:conditions` key (NeoForge's condition format):
...
```json
{
  ...
  "scales": { "pehkui:width": 0.5 },
  "neoforge:conditions": [
    { "condition": "neoforge:mod_loaded", "modid": "some_mod" }
  ]
}
```

Common NeoForge conditions: `neoforge:mod_loaded` (`modid`), `neoforge:and` / `neoforge:or` / `neoforge:not` (combinators), `neoforge:tag_empty`, `neoforge:registered`.

For cross-loader datapacks, the Fabric `fabric:load_conditions` format is also honored on NeoForge for `all_mods_loaded` / `any_mods_loaded`.
...
All knobs live in `config/pehkui/config.json` (editable in-game: **Fabric** via ModMenu → Pehkui, **NeoForge** via the mod list → Pehkui → config). Full minimal template:
