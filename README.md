...
- Minecraft versions: `1.21.2` ~ `1.21.4`
...
Each supported Minecraft version family lives on its own branch (e.g. `neoforge/1.21.4`). Branches are **single-main-version skeletons**: built against the latest patch of the family, with the mod metadata declaring a version range covering the whole supported series (e.g. `1.21.2` through `1.21.4`). The mixin layer contains only the injection points that apply to that version family — no cross-version `compat*` subpackages, no legacy version-gating infrastructure. All mixins are flattened into `mixin/` (server) and `mixin/client/`.
